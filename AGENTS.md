# AGENTS.md — How to use agent-radar
...
1. **On any "what's the status?" or progress question, first run the board and answer from it.**
   ...
   # Do this:
   radar board
   # (or radar get "the relevant name")
   # Then base your reply on the returned items. Do NOT re-derive from chat history alone.
   ```

2. **Use `radar set` to communicate state.** Never just say it in prose if it belongs on the board.
...
6. **Set the right status (see semantics above):**
   ...
   - `done` — finished (excluded from normal `radar board` reads).

7. **Source attribution happens automatically** from `RADAR_SOURCE` (or fallback `DOODLE_SOURCE`) or `AGENT_NAME` env. Set it in your shell/session so humans know who wrote it.

## Single-writer model (conductor curates)

The board is **single-writer** in practice: the conductor is the curator who owns the overall state. Individual sessions/agents **report** status, questions, and blockers to the conductor (via `radar set`), rather than directly editing or assuming ownership of the board.
...
radar set "Auth middleware" --type session --status active --summary "Implementing rate limiting" --detail "Chose token bucket. Any org patterns?"

# When you need the human
radar set "Auth middleware" --status waiting_on_user --summary "Rate limiting design" --detail "Token bucket vs fixed window + existing patterns?"

# Later, after user answers or you make progress
radar set "Auth middleware" --status active --summary "Proceeding with token bucket (per user)"

# When finished
radar set "Auth middleware" --status done --summary "Rate limiting complete"
```

## Reading (Critical)

```bash
radar board                 # default: current actionable items as JSON
radar board --pretty        # human readable (you can show user)
radar board --status waiting_on_user
radar get "auth middleware"
```
...
Every `radar board` prints this footer (on stderr):
...
- `RADAR_BOARD_PATH` (primary; falls back to `DOODLE_BOARD_PATH` for one release) — use a project-specific board if desired.
- `RADAR_SOURCE` (primary; falls back to `DOODLE_SOURCE`) — your identity (e.g. "conductor", "search-agent", "reviewer-3").
